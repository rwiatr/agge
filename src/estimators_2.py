...
class UnaryEstimator:
    ...
    # def bin_vector(self, df, bin_count=None, bin_type='cnt'):
    ...
    def predict(self, df, bin_count=None, bin_type='cnt'):
        ...
        if bin_count is not None and bin_count > 0:
            bins, mean_bin_count = f_bins(self.model[self.value_column] / self.model['estimator_event_col'],
        ...
        for bin_id in range(int(max_bin + 1)):
            bin_column = pcolumn + '_bin=' + str(bin_id) + '|' + str(mean_bin_count[bin_id])
