...
from experiment.ipinyou.onehot.algo import SKLearnMLPRunner, SKLearnLRRunner, MLPRunner, DeepWideRunner, \
    DeepWideRunnerV2, DampReg, RegV2, DmpRegV2, DmpRegV3, DmpRegTMP
...
def calc_agg():
    DmpRegV3(agge_ds[0]['counts'].copy(), experiment_id).set_measure(measure) \
        .run(*agge_ds,
             subject + f";encoding=agge-dmp2;features="
                       f"{agge_ds[0]['train'].shape[1]};"
    ...
    ##################################################
    DmpRegV2(agge_ds[0]['counts'].copy(), experiment_id).set_measure(measure) \
        .run(*agge_ds,
             subject + f";encoding=agge-dmp;features="
                       f"{agge_ds[0]['train'].shape[1]};"
                       f"conj=False;bins={bins}",
             a=1.0, **nn_params)
...
def calc_agg_conj():
    DmpRegV3(ohe_conj_agge[0]['counts'].copy(), experiment_id).set_measure(measure) \
        .run(*ohe_conj_agge,
             subject + f";encoding=oh+agge-dmp2;features="
                       f"{ohe_conj_agge[0]['train'].shape[1]};"
                       f"conj=True;bins={bins}",
             a=0, **nn_params)
    #
    # # sanity
    # DmpRegV2(ohe_conj_agge[0]['counts'].copy(), experiment_id).set_measure(measure) \
    #     .run(*ohe_conj_agge,
    #          subject + f";encoding=oh+agge-dmp;features="
    #                    f"{ohe_conj_agge[0]['train'].shape[1]};"
    #                    f"conj=True;bins={bins}",
    #          a=0.0, **nn_params)
...
def calc_oh():
    RegV2(experiment_id).set_measure(measure).run(*ohe_conj_ht,
                                                  subject + f";encoding=oh+ht-raw;features="
                                                            f"{ohe_conj_agge[0]['train'].shape[1]};"
                                                            f"conj=True;bins={bins}",
                                                  **nn_params)
    RegV2(experiment_id).set_measure(measure).run(*ohe_dataset,
                                                  subject + f";encoding=oh-raw;features="
                                                            f"{ohe_dataset[0]['train'].shape[1]};"
                                                            f"conj=False;bins={bins}",
                                                  **nn_params)
...
if __name__ == '__main__':
    ...
    experiments = generate_space([
        ...
        ['3386', '2261', '2997', '2821', '3476', '3427', '3386'],  # , '2821', '3358', '3386', '3427', '3476'],
    ...
    for experiment_id, (subject, sample_id, bins, alpha, hidden) in experiments:
        ...
        ohe_conj_ht = None
        ohe_conj_agge = None
        X, y = d_mgr.get_data(subject, bins, sample_id, conj=conj, agge=True, agge_conj=True, ht=True)
